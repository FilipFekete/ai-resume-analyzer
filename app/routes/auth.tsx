...
const Auth = () => {
  ...
  // Track previous auth state to detect fresh login on this page
  const wasAuthedRef = useRef(auth.isAuthenticated);

  // If already authenticated and not in manage mode, redirect immediately
  useEffect(() => {
    if (auth.isAuthenticated && !manage) {
      navigate(next, { replace: true });
    }
  }, [auth.isAuthenticated, manage, next, navigate]);

  // If user logs in while on this page (false -> true), redirect even in manage mode
  useEffect(() => {
    const wasAuthed = wasAuthedRef.current;
    if (!wasAuthed && auth.isAuthenticated) {
      navigate(next, { replace: true });
    }
        ...
        <section className="flex items-center justify-center px-8 py-12">
          <blockquote className="max-w-2xl text-center lg:text-left">
            <p className="text-3xl lg:text-4xl font-extrabold text-gray-900 leading-tight">
              “Your CV is a movie trailer for your career—trim the filler, amplify the highlights, and leave recruiters eager for the premiere.”
            </p>
